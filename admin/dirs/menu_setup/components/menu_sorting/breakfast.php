<div class="justify-content-end mr-4 d-flex mt-2 gap-2">
	<div class="input-group border bg-light px-3 flex-grow-1" style="max-width: 300px;">
	    ...
	    <input type="search" id="search-breakfast" class="form-control bg-transparent border-0 small py-2 shadow-none" placeholder="Search...">
	</div>
	<button class="btn btn-primary" type="button" onclick="addBreakFast()"><i class="bi bi-plus-lg"></i> Add Break Fast</button>
</div>

<!-- Load Break Fast Content -->
<div class="card mt-2">
	<div class="card-body p-0">
	    <div class="table-responsive overflow-auto" style="height: 50vh;">
	        <table class="table table-hover align-middle mb-0">
	            ...
	            <tbody class="border-top-0" id="load_Breakfast_content">
	            </tbody>
	        </table>
	    </div>
	</div>
	<div class="card-footer">
	    <nav>
	        <ul class="pagination" id="pagination-breakfast">
	            <li class="page-item" id="li-prev-breakfast">
	                <a class="page-link" href="#" id="btn-preview-breakfast">Previous</a>
	            </li>
	            <li class="page-item" id="li-next-breakfast">
	                <a class="page-link" href="#" id="btn-next-breakfast">Next</a>
	            </li>
	        </ul>
	    </nav>
	    <div id="page-info-breakfast" class="mt-3 small text-muted"></div>
	</div>
</div>
...
<script>
	...
	function loadBreakfast(page = 1) {
	    CurrentPage = page; 
	    var display = $("#load_Breakfast_content");
	    display.html(`
	            <tr>
	    ...
	    `);
	    var Search = $("#search-breakfast").val();
	    $.post("dirs/menu_setup/actions/get_pagination_breakfast.php", {
	        CurrentPage,
	        PageSize,
	        Search
	    }, function (data) {
	        ...
	        if ($.trim(response.isSuccess) === "success") {
	            BreakFastContent(response.Data);
	            totalPages = (response.Data && response.Data.length > 0)
	                ? parseInt(response.Data[0].TotalPages)
	                : 1;

	                BreakFastPageNumber();
	                BreakFastPaginationUi();
	        } else {
	            emptyStateBreakfast("breakfast menu was empty.");
	        }
	    });
	}


	function BreakFastContent(data) {
	    const display = $("#load_Breakfast_content");

	    if (!data || data.length === 0) {
	        showEmptyStateBreakfast("No menu available.");
	        return;
	    }
	    ...
	    data.forEach(bfast => {
	        display.append(`
	           <tr class="breakfast-row align-middle" data-value="${bfast.LineNum}">
	               ...
	               <td class="text-muted text-center small">
	        	 	...
	        	 	<td class="text-center">
	        	 	    <span class="badge px-3 py-2 rounded-pill toggle-status cursor-pointer
	        	 	        ${bfast.DishStatus === "Active" ? "bg-success-subtle text-success" : "bg-danger-subtle text-danger"}"
	        	 	        data-id="${bfast.LineNum}"
	        	 	        data-status="${bfast.DishStatus}">
	        	 	        ${bfast.DishStatus}
	        	 	    </span>
	        	 	</td>
	               ...
	               <td class="text-center">
	                   <div class="dropdown">
	                       ...
	                       <ul class="dropdown-menu dropdown-menu-end shadow-sm">
	                           <li>
	                               <a class="dropdown-item d-flex align-items-center gap-2" href="#" onclick="mdleditBreakfast('${bfast.LineNum}')">  <i class="bi bi-pencil text-primary"></i>  Edit
	                               </a>
	                           </li>
	                           <li>
	                               <a class="dropdown-item d-flex align-items-center gap-2 text-danger"href="#"onclick="removemdlBreakfast('${bfast.LineNum}')"> <i class="bi bi-trash"></i> Remove
	                               </a>
	                           </li>
	                           ...
	                           <li>
	                               <a class="dropdown-item d-flex align-items-center gap-2" href="#" onclick="enableBreakfast('${bfast.LineNum}')">  <i class="bi bi-toggle-on text-success"></i>  Enable
	                               </a>
	                           </li>

	                           <li>
	                               <a class="dropdown-item d-flex align-items-center gap-2" href="#" onclick="disableBreakfast('${bfast.LineNum}')">  <i class="bi bi-toggle-off text-secondary"></i>  Disable
	                               </a>
	                           </li>
	                       </ul>
	                   </div>
	...
	function emptyStateBreakfast(message) {
	    $("#load_Breakfast_content").html(`
	        <tr>
	          <td colspan="6" class="p-5 text-center text-muted">
	...
	/*Function for no record of beverages*/
	function showEmptyStateBreakfast(message) {
	    $("#load_Breakfast_content").html(`
	        <tr>
	          <td colspan="6" class="p-5 text-center text-muted">
	              <i class="bi bi-card-list"></i> 
	...
	function BreakFastPaginationUi() {
	    $("#page-info-breakfast").text("Page " + CurrentPage + " of " + totalPages);
	    if (CurrentPage <= 1) {
	        $("#li-prev-breakfast").addClass("disabled");
	    } else {
	        $("#li-prev-breakfast").removeClass("disabled");
	    }
	    if (CurrentPage >= totalPages) {
	        $("#li-next-breakfast").addClass("disabled");
	    } else {
	        $("#li-next-breakfast").removeClass("disabled");
	    }
	}
	/*Function to build list of pagination*/
	function BreakFastPageNumber() {
	    $("#pagination-breakfast li.page-number-breakfast").remove();
	    let prevLi = $("#li-prev-breakfast");
	    let maxVisible = 5;
	    ...
	    if (start > 1) {
	        ...
	        if (start > 2) {
	            prevLi.after(`<li class="page-item page-number-breakfast disabled"><span class="page-link">...</span></li>`);
	            prevLi = prevLi.next();
	        }
	    }
	    ...
	    function insertPage(i, ref) {
	        ...
	        let li = `
	            <li class="page-item page-number-breakfast ${activeClass}">
	...
	/*search-breakfast*/
	$("#search-breakfast").on("keydown", function(e) {
	    if (e.key === "Enter") {
	        loadBreakfast();
	    }
	});
	  ...
	  $("#btn-preview-breakfast").on("click", function(e) {
	      e.preventDefault();

	      if (CurrentPage > 1) {
	          loadBreakfast(CurrentPage - 1);
	      }
	  });

	/*Function load all important tags tickets*/
	  $("#btn-next-breakfast").on("click", function(e) {
	      e.preventDefault();
	  ...
	  function mdleditBreakfast(LineNum){
	  ...
	  /*Function update breakfast details*/
	    ...
	    function removemdlBreakfast(LineNum) {
	        Swal.fire({
	    ...
	    function enableBreakfast(LineNum){
	      var Status = 'Active';
	    ...
	    /*Function enable menu*/
	    function disableBreakfast(LineNum){
